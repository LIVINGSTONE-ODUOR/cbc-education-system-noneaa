...
interface ExamFormState {
  id: string | null;
  year: string; // "Academic Year" — the distinct `year` grouping a term belongs to
  term_id: string;
  grade_level: string;
  class_id: string;
  exam_name: string;
...
const ExamSetup: React.FC = () => {
  ...
  const loadExams = useCallback(async () => {
    ...
    try {
      ...
      toast.error('Could not load exams. Check your connection and try again.');
    } finally {
    ...
    <div className="min-h-screen p-6 space-y-6 bg-gray-50">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button
            ...
            disabled={loadingReferenceData || classes.length === 0 || terms.length === 0}
          >
      ...
      {!loadingReferenceData && (classes.length === 0 || terms.length === 0) && (
        <Card className="border-amber-200 bg-amber-50">
          <CardContent className="flex items-center gap-3 py-4 text-sm text-amber-800">
            ...
            <span>
              {classes.length === 0 &&
                'No classes found for your school — add classes under Administration → Classes before creating exams. '}
              {terms.length === 0 &&
                'No academic years / terms found — add a term under Administration → Term Management before creating exams.'}
            </span>
          </CardContent>
        </Card>
      )}
      ...
      <Card className="rounded-2xl border border-gray-200 shadow-sm">
        <CardHeader>
          ...
          <CardDescription>All examinations created for your school</CardDescription>
        </CardHeader>
        <CardContent>
          {loadingExams ? (
            ...
            <div className="flex flex-col items-center justify-center py-16 text-center text-gray-500">
              ...
              <p className="text-sm">
                {search || filterExamType !== 'all' || filterTermId !== 'all' || filterStatus !== 'all'
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1.5">
                ...
                <Select
                  value={form.year}
                  onValueChange={(value) =>
                    setForm((f) => ({ ...f, year: value, term_id: '' }))
                  }
                >
                  <SelectTrigger className="rounded-xl">
              ...
              <div className="space-y-1.5">
                ...
                <Select
                  ...
                  onValueChange={(value) =>
                    setForm((f) => ({ ...f, grade_level: value, class_id: '' }))
                  }
                >
                  <SelectTrigger className="rounded-xl">
      ...
      <Dialog open={showViewDialog} onOpenChange={setShowViewDialog}>
        <DialogContent className="sm:max-w-md rounded-2xl">
          ...
          {selectedExam && (
            <div className="space-y-3 text-sm">
              ...
              <div className="flex justify-between border-b border-gray-100 pb-2">
                ...
                <span className="font-medium text-gray-900">{formatDate(selectedExam.end_date)}</span>
              </div>
              <div className="flex justify-between border-b border-gray-100 pb-2">
          ...
          <DialogFooter>
            <Button variant="outline" className="rounded-xl" onClick={() => setShowViewDialog(false)}>
              Close
            </Button>
          </DialogFooter>
        </DialogContent>
      ...
      <Dialog open={showDeleteDialog} onOpenChange={setShowDeleteDialog}>
        <DialogContent className="sm:max-w-sm rounded-2xl">
          ...
          <p className="text-sm text-gray-600">
            ...
            <span className="font-bold text-gray-900">{selectedExam?.exam_name}</span>? This
            cannot be undone.
          </p>
